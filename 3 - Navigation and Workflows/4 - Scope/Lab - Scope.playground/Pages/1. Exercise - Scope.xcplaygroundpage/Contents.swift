...
//print("The value of foo is \(foo)")
//Because the constant foo has a local scope so it is not accessable outside the loop
...
//Here x is a global variable declared outside the loop so it has access all over and it gets updated whereas foo was declared inside which makes it local variable
...
greeting(greeting: "Good Morning", name: "Sara")
greeting(greeting: nil, name: "Parul")
